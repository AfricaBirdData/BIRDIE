...
ui <- fluidPage(
    ...
    sidebarLayout(
        ...
        sidebarPanel(
            numericInput('sp_code', 'SAFRING code', min = 1, max = 10000, value = 4, step = 1),
...
server <- function(input, output) {
    ...
    set_config <- function(input){
        configPreambAbu(input$year, server = FALSE, mod_file = "")
    }
    ...
    get_fit <- function(input, config){
        readRDS(paste0("../../../",setSpOutFilePath("ssm_fit", config, input$sp_code, ".rds")))
    }


    # Load data
    get_counts <- function(input, config){
        read.csv(paste0("../../../", setSpOutFilePath("abu_model_data", config, input$sp_code, ".csv")))
    }
    ...
    load_fit <- reactive({
        ...
        fit <- get_fit(input, config)
        # compute stats
        BIRDIE:::processJAGSoutput(fit, DIC = FALSE, params.omit = NULL)
    }) %>%
    ...
    observe({
        output$tracePlot <- renderPlot({
            ...
            mcmc_trace(fit, pars = vars(contains(site_param)))
...
#             counts <- load_counts()
            ...
            post_sims <- postPredDistJagsSsm(fit_stats, counts, 500)
