...
#'
#' @examples
ppl_run_pipe_distr <- function(sp_code, year, config,
    ...
    if("fit" %in% steps){
        ppl_fit_occur_model(sp_code, year, ...)
    }

    if("summ" %in% steps){
        ppl_summarize_occur(sp_code, year, ...)
    }
    ...
    if("indtr" %in% steps){
        ...
        for(t in seq_along(config$years)){
            ...
            if((t > config$dyear/2) | (config$year < (2009 + config$dyear/2))){
                ppl_estimate_aoo(sp_code, year = config$years[t], ...)
            }
        ...
        if(year != 2017){
            for(t in seq_along(config$years[-1])){
                ...
                if(config$years[t+1] > 2008){
                    ppl_estimate_daoo(sp_code, year = config$years[t+1], term = "annual", ...)
                }
            ...
            for(t in seq_along(config$years)){
                ...
                if(config$years[t] > 2017){
                    ppl_estimate_daoo(sp_code, year = config$years[t], term = "short", ...)
                }
