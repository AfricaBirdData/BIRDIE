...
#' fit <- fitCwacCtSsm(ssmcounts, param = c("beta", "lambda", "sig.zeta", "sig.w", "sig.eps", "sig.alpha", "sig.e", "mu_t", "mu_wt"))
#' plotCtSsm(fit = fit, ssm_counts = ssmcounts, dyn = FALSE)
plotCtSsm <- function(fit, ssm_counts, dyn = FALSE){
    ...
    stt_plot <- post_stt %>%
        ...
        ggplot2::xlab("Time") + ggplot2::ylab("log-abundance")
