...
#' (EXPERIMENTAL)
#'
#' @param fit A continuous-time JAGS state-space model fitted to CWAC data
#' @param ssm_counts A data frame with the count data use to fit the
#' state-space model
#' @param dyn Whether the fitted long-term trend is fixed or dynamic
#' (NOT USED AT PRESENT).
#'
#' @return A list with two elements: i) plot: a plot with summer and winter
...
#'
#' @examples
plotCtSsm <- function(fit, ssm_counts, dyn = FALSE){

    # Create a data frame with the posterior state
    post_stt <- data.frame(mu_est = fit$mean$mu,
                           mu_lb = fit$q2.5$mu,
                           mu_ub = fit$q97.5$mu,
                           t = unique(ssm_counts$t),
    ...
    stt_plot <- post_stt %>%
        tidyr::pivot_longer(cols = c(mu_est, mu_lb, mu_ub),
                            names_to = "quantile") %>%
        ggplot2::ggplot() +
        ggplot2::geom_path(aes(x = t, y = value, linetype = quantile)) +
        ggplot2::geom_point(aes(x = t, y = count, col = factor(season)), show.legend = FALSE) +
        ...
        ggplot2::facet_wrap("season", nrow = 3,
                            labeller = ggplot2::labeller(season = c("S" = "Summer", "W" = "Winter", "O" = "Other"))) +
        ggplot2::xlab("t") + ggplot2::ylab("log-abundance")
    ...
    #
        ...
        trd_plot <- post_trd %>%
            ...
            ggplot2::geom_path(aes(x = t, y = value, linetype = quantile)) +
            ggplot2::scale_linetype_manual(name = "", values = c(1, 2, 2), guide = NULL) +
            ggplot2::xlab("t") + ggplot2::ylab("log growth-rate")
        ...
        prop_plot <- post_trd %>%
            ...
            ggplot2::geom_path(aes(x = t, y = value, linetype = quantile)) +
            ggplot2::scale_linetype_manual(name = "", values = c(1, 2, 2), guide = NULL) +
            ggplot2::xlab("t") + ggplot2::ylab("Log proportion")
    # }
