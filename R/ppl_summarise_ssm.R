...
#'
#' @param fit A model fit generated by \code{\link{ppl_fit_ssm_model}}
...
#' @param linear If TRUE (default) then summaries are given the linear scale.
#' If FALSE, then summaries are given in the modelling scale (typically log scale)
...
#'
#' @examples
ppl_summarise_ssm <- function(fit, counts, sp_code, linear = TRUE, config, ...){
    ...
    p <- BIRDIE::plotJagsSsm2ss(fit = fit_stats, ssm_counts = counts, linear = linear,
    ...
    for(i in seq_along(out_all_sites)){
        ...
        plotdir <- setSpOutFilePath("ssm_plot", config, config$years_ch, sp_code, paste0("_", site_sel, "_id_", i, ".png"))
