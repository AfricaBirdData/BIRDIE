...
source("comms/indicators/createGraphDf.R")
...
source("comms/indicators/createGraph.R")
...
source("comms/indicators/combineGraphs.R")

graph_comb <- combineGraphs(list(sp_gr, sit_gr, sptpe_gr, sitpe_gr))

render_graph(graph_comb)

export_graph(graph_comb, file_name = "comms/output/all_graph_comb.png", file_type = "PNG")
...
# Find one route
source("comms/indicators/createConditionGraph.R")

render_graph(createConditionGraph(ind_df, c("at", "across", NA), c("site_type", "sp_type", NA), output = "graph"))

# Find all routes
source("comms/indicators/createAllConditionGraphs.R")
...
export_graph(graph_comb, file_name = "comms/output/site_graph_comb.png", file_type = "PNG")
