...
The folder *analysis* contains data, scripts and outputs from analyses related to the BIRDIE project. This folder is not part of the package, so nothing in there is meant to be installed. Note that the *analysis/out_nosync* folder does not sync to avoid accumulating too much (potentially large) output that is likely to change frequently. Use this folder for temporary, intermediate products and the *output* folder for more permanent or shareable outputs.
