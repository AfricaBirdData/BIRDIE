...
that are performed during the first step of the DST module: **data
preparation**.
...
two separate datasets from [ABAP](https://www.birdmap.africa/) data: one
with one entry for each site (pentad) visited, and one with one entry
for each visit conducted (i.e., pentads can be visited more than once).
In fact, this function will create a third dataset. For any particular
year, sites and visits will be common for all species, so we need a
third dataset with the detection information for each species.
...
Then, the main pipeline function
[`ppl_run_pipe_dst1()`](https://africabirddata.github.io/BIRDIE/reference/ppl_run_pipe_dst1.md)
gives us the option to select which year we want to run models for with
the argument `year` and we may skip data preparation by setting
`force_gee_dwld = FALSE`, `force_site_visit = FALSE`, like so
...
This process is facilitated by the use of the [`ABAP` R
package](https://github.com/AfricaBirdData/ABAP).

There isn’t much to say that is not explained on the GitHub repository
of the ABAP package (check it out).

We will need to call the functions
[`ABAP::getAbapData()`](https://rdrr.io/pkg/ABAP/man/getAbapData.html)
and
[`ABAP::getRegionPentads()`](https://rdrr.io/pkg/ABAP/man/getRegionPentads.html)
several times during the data preparation process.

## Annotate with environmental covariates

We use environmental covariates to model occupancy and detection
...
datasets and functionality offered by Google Earth Engine (GEE).
...
functionality in the pipeline. There are two functions in BIRDIE that
are used to annotate ABAP data:
...
which are used to annotate site and visit data, respectively. See
`?prepGEESiteData()` and `?prepGEEVisitData()` for details. All GEE
related functions have been packaged in the file `R/utils-gee.R`
...
and
[`prepGEEVisitData()`](https://africabirddata.github.io/BIRDIE/reference/prepGEEVisitData.md)
functions. This is not ideal, but it is how it is set up currently. We
also have to keep this in mind when we pass the covariates we want to
use in the models to the
[`configPipeline()`](https://africabirddata.github.io/BIRDIE/reference/configPipeline.md)
function in the control script (see *BIRDIE-spp-distributions*). These
covariates must be among those provided by
[`prepGEESiteData()`](https://africabirddata.github.io/BIRDIE/reference/prepGEESiteData.md)
and
[`prepGEEVisitData()`](https://africabirddata.github.io/BIRDIE/reference/prepGEEVisitData.md)
...
This now brings us to the next part of the data preparation, which is
the formatting of site, visit and detection data.
...
specified in
[`configPipeline()`](https://africabirddata.github.io/BIRDIE/reference/configPipeline.md).
Data coming from GEE will be in a wide format, meaning that each
variable and year will be in a separate column. In general, we would
like variables to be in one column and years in another column. This is
one of the important tasks
[`createOccuData()`](https://africabirddata.github.io/BIRDIE/reference/createOccuData.md)
will do for us. We also use this function to create transformations of
those variables coming from GEE (e.g. we sometimes use log
transformations) and interactions.
**[`createOccuData()`](https://africabirddata.github.io/BIRDIE/reference/createOccuData.md)
is also hard-coded**, so if we decide to use some new variable
transformation, we need to modify this function to create it explicitly.
Interactions should be handled correctly as long as the variables
involved in the interaction are present in the data.
